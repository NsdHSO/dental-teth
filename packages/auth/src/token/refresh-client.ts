 ...
 * SOLID: Dependency Inversion - uses @dental/http abstractions.
 */
import axios, { AxiosInstance } from 'axios';
import { applyEnvelopeUnwrapper, AUTH_BASE } from '@dental/http';
...
export class TokenRefreshClient implements ITokenRefreshClient {
  ...
  private extractRefreshToken(data: RefreshResponse): string | undefined {
    return (
      data.refreshToken ?? data.refresh_token ?? data.message?.refresh_token
    );
  }
}
