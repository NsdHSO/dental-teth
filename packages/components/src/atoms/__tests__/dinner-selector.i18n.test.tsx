...
describe('DinnerSelector i18n', () => {
  ...
  beforeEach(async () => {
    jest.clearAllMocks();
    const { initI18n } = require('@dental/i18n');
    await initI18n();
  });
  ...
  describe('Dinner fallback label', () => {
    ...
    it('should show location when available instead of fallback', () => {
      ...
      // Dinner #1 has location 'detal-teth Hall'
      const dinnerItem = screen.getByTestId('picker-item-1');
      expect(dinnerItem.props.children).toContain('detal-teth Hall');
    });
