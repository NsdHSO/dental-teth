...
export class HttpDentistsRepository implements DentistsRepository {
  ...
  async update(id: number, body: UpdateDentistInput): Promise<Dentist> {
    return toDentist(
      await unwrap<DentistDto>(appApi.put(`/dentists/${id}`, body)),
    );
  }
  ...
  async autocomplete(
    q: string,
    limit = 10,
  ): Promise<DentistAutocompleteItem[]> {
    const res = await unwrap<DentistAutocompleteItemDto[]>(
      appApi.get(
        `/dentists/autocomplete?q=${encodeURIComponent(q)}&limit=${limit}`,
      ),
    );
...
export const defaultDentistsRepository: DentistsRepository =
  new HttpDentistsRepository();
