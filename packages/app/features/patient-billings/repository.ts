...
export interface PatientBillingsRepository {
    ...
    update(patientId: number, billingId: number, body: UpdateBillingInput): Promise<Billing>;
...
export class HttpPatientBillingsRepository implements PatientBillingsRepository {
    async list(patientId: number, params?: ListBillingsParams): Promise<BillingsList> {
    ...
    async create(patientId: number, body: CreateBillingInput): Promise<Billing> {
        return toBilling(
            await unwrap<BillingDto>(appApi.post(`/patients/${patientId}/billings`, body)),
        );
    }
    ...
    async markPaid(patientId: number, billingId: number): Promise<Billing> {
        return toBilling(
            await unwrap<BillingDto>(
                appApi.put(`/patients/${patientId}/billings/${billingId}/mark-paid`, {}),
            ),
        );
    }
}
