...
export class HttpPatientsRepository implements PatientsRepository {
  ...
  async update(id: number, body: UpdatePatientInput): Promise<Patient> {
    return toPatient(
      await unwrap<PatientDto>(appApi.put(`/patients/${id}`, body)),
    );
  }
  ...
  async autocomplete(
    q: string,
    limit = 10,
  ): Promise<PatientAutocompleteItem[]> {
    const res = await unwrap<PatientAutocompleteItemDto[]>(
      appApi.get(
        `/patients/autocomplete?q=${encodeURIComponent(q)}&limit=${limit}`,
      ),
    );
...
export const defaultPatientsRepository: PatientsRepository =
