...
import { Pagination, Role, RoleDto, RoleInput, RolesList } from './types';
import { toDtoInput, toRole, toRolesList } from './mapper';
...
export class HttpRolesRepository implements RolesRepository {
  async list(params?: ListParams): Promise<RolesList> {
    ...
    const res = await unwrap<{ data: RoleDto[]; pagination: Pagination }>(
      appApi.get(`/roles?${q.toString()}`),
    );
  ...
  async create(body: RoleInput): Promise<Role> {
    return toRole(
      await unwrap<RoleDto>(appApi.post('/roles', toDtoInput(body))),
    );
  }

  async update(id: number, body: Partial<RoleInput>): Promise<Role> {
    return toRole(
      await unwrap<RoleDto>(
        appApi.put(`/roles/${id}`, toDtoInput(body as RoleInput)),
      ),
    );
  }
...
export const defaultRolesRepository: RolesRepository =
  new HttpRolesRepository();
