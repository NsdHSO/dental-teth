...
export class HttpAppointmentsRepository implements AppointmentsRepository {
    async list(params?: ListAppointmentsParams): Promise<AppointmentsList> {
        const { page = 1, limit = 20, date, from, to, dentist_id, patient_id, status } = params ?? {};
...
export class MockAppointmentsRepository implements AppointmentsRepository {
    ...
    constructor({
        ...
        this.data = seed.map(s => ({
            ...
            time: s.time,
            dentist: s.dentist,
            patient: { id: s.patient?.id ?? 0, fullName: s.patient?.fullName ?? null, phone: null, email: null },
            reason: s.reason,
    ...
    async list(params?: ListAppointmentsParams): Promise<AppointmentsList> {
        ...
        if (params?.dentist_id) {
            filtered = filtered.filter((a) => a.dentist === `Dentist #${params.dentist_id}`);
        }
    ...
    async create(body: AppointmentInput): Promise<Appointment> {
        ...
        const appointment: Appointment = {
            ...
            patient: { id: body.patient_id, fullName: null, phone: null, email: null },
    ...
    async update(
        ...
        this.data[idx] = {
            ...
            dentist: body.dentist_id ? `Dentist #${body.dentist_id}` : existing.dentist,
