...
config.resolver = {
  ...
  resolveRequest: (context, moduleName, platform) => {
    // Force @dental/* packages to use source (react-native field) instead of dist
    if (moduleName.startsWith('@dental/')) {
    ...
    // Let Metro resolve SVG files from @dental/* packages
    if (
      moduleName.endsWith('.svg') &&
      context.originModulePath.includes('@dental')
    ) {
