...
import { authApi, setTokensFromLogin } from '@dental/auth';
...
export function useRegisterForm() {
  ...
  async function onSubmit() {
    if (!email || !password)
      return Alert.alert(
        t('common.missingFields'),
        t('auth.register.missingFields'),
      );
    if (password !== confirm)
      return Alert.alert(
        t('auth.register.passwordMismatchTitle'),
    ...
    try {
      ...
      Alert.alert(
        t('common.error'),
        typeof msg === 'string' ? msg : t('auth.register.errorGeneric'),
      );
    } finally {
      setSubmitting(false);
    }
