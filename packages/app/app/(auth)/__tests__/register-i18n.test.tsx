...
jest.mock('@dental/auth', () => ({
  authApi: { post: (...args: any[]) => mockAuthApiPost(...args) },
  setTokensFromLogin: (...args: any[]) => mockSetTokensFromLogin(...args),
}));
...
const authApi = { post: mockAuthApiPost };
...
describe('RegisterScreen - i18n Migration', () => {
  ...
  describe('Alert strings - password mismatch', () => {
    it('should use t() for password mismatch alert', () => {
      ...
      fireEvent.changeText(
        getByPlaceholderText('auth.register.emailPlaceholder'),
        'test@test.com',
      );
      fireEvent.changeText(
        getByPlaceholderText('auth.register.passwordPlaceholder'),
        'password1',
      );
      fireEvent.changeText(
        getByPlaceholderText('auth.register.confirmPassword'),
        'password2',
      );
  ...
  describe('Alert strings - registration error', () => {
    it('should use t() for error alert title', async () => {
      mockAuthApiPost.mockRejectedValue(new Error('Server error'));
      ...
      // Find and toggle the terms acceptance Switch
      const switchElement = UNSAFE_root.findAllByType('RCTSwitch')[0];
    ...
    it('should use t() for generic error fallback when message is non-string', async () => {
      mockAuthApiPost.mockRejectedValue({
