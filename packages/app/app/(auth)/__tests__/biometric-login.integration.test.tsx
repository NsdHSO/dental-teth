...
describe('Biometric Login Flow - Integration Tests', () => {
    ...
    describe('Biometric Button Visibility', () => {
        ...
        it('should not show biometric button when hardware exists but preference is disabled', async () => {
            ...
            await waitFor(() => {
                expect(auth.getBiometricPreference).toHaveBeenCalled();
            });
        ...
        it('should allow password login after cancelling biometric', async () => {
            ...
            await waitFor(() => {
                expect(mockPost).toHaveBeenCalledWith('/auth/login', {
                    ...
                    notes: JSON.stringify({client: "detal-teth", email: 'user@test.com'})
                });
            });
    ...
    describe('Edge Cases', () => {
        it('should handle biometric availability check error gracefully', async () => {
            (auth.isBiometricAvailable as jest.Mock).mockRejectedValue(
        ...
        it('should handle getBiometricPreference error gracefully', async () => {
            (auth.isBiometricAvailable as jest.Mock).mockResolvedValue(true);
            (auth.getBiometricPreference as jest.Mock).mockRejectedValue(
        ...
        it('should not allow multiple concurrent biometric sign in attempts', async () => {
            ...
            await waitFor(() => {
                expect(auth.authenticateWithBiometrics).toHaveBeenCalledTimes(1);
            });
        ...
        it('should allow password login after authenticateWithBiometrics throws', async () => {
            (auth.isBiometricAvailable as jest.Mock).mockResolvedValue(true);
            (auth.getBiometricPreference as jest.Mock).mockResolvedValue(true);
            (auth.getBiometricEmail as jest.Mock).mockResolvedValue('bio@test.com');
            (auth.authenticateWithBiometrics as jest.Mock).mockRejectedValue(
    ...
    describe('AuthProvider signInWithBiometrics', () => {
        ...
        it('should complete biometric sign in through AuthProvider', async () => {
            ...
            await waitFor(() => {
                expect(auth.getBiometricEmail).toHaveBeenCalled();
            });
        ...
        it('should handle special characters in credentials', async () => {
            ...
            await waitFor(() => {
                expect(mockPost).toHaveBeenCalledWith('/auth/login', {
                    ...
                    password: 'P@ssw0rd!@#$%^&*',
                    notes: JSON.stringify({client: "detal-teth", email: 'test+special@sub.example.com'})
                });
            });
        });
    });
